      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {widgets.map((w, i) => (
          <div key={i} className="rounded border p-3">
            <p className="text-sm text-gray-500 mb-2">{w.title}</p>
            {renderWidget(w)}
          </div>
        ))}
      </div>
...
function renderWidget(w: Widget) {
  ...
  switch (w.kind) {
    case "stat_card":
      return <p className="text-3xl font-bold">{String((data[0] as any)?.value ?? "—")}</p>;
    case "line_chart":
      return (
            ...
            {data.map((row, i) => (
              <tr key={i}>
                {Object.values(row).map((v, j) => (
                  <td key={j} className="border-t py-1">{String(v)}</td>
                ))}
              </tr>
            ))}
          </tbody>
